...
自訂 Alias、QR Code、會員系統及完整流量分析並非題目的必要條件，因此先保留為後續擴充，避免 MVP 因範圍過大而延遲。
...
這不代表所有系統都不需要前後端分離。若未來有手機 App、多個前端或獨立擴展後端的需求，再拆成不同服務會更合理；但在目前需求下，Nuxt 全端架構是開發效率與可維護性之間較合適的選擇。
...
MVP 的核心資料表為 `short_urls`，預計保存 `id`、`original_url`、具唯一限制的 `code`、`click_count` 與 `created_at`。其中 `code` 需要建立 Unique Index，既能阻止重複短碼，也能加速 Redirect 時最頻繁的查詢。若未來需要分析來源、裝置或每日流量，再增加 `click_logs`，不讓第一版承擔過早的資料複雜度。
...
`POST /api/urls` 接收原始網址並回傳短網址；`GET /:code` 找到資料後增加點擊數，再使用 `302` Redirect；統計 API 則回傳原始網址、建立時間與點擊次數。若輸入格式錯誤會回傳 `400`，短碼不存在則回傳 `404`。
...
完成 MVP 後，再評估部署至雲端平台，並視時間錄製實作過程。這套規劃的核心不是堆疊最多技術，而是在有限範圍內交付一個功能完整、結構清楚，也能繼續擴充的縮網址網站。
